...
public static partial class UIToolKitExtensions
{
    ...
    /// <li>Supports multiple classes separated by spaces: USS("h-40 w-40")</li>
    /// </summary>
    ...
    /// <inheritdoc cref="USS{T}(IEnumerable{T},string)"/>
    public static IEnumerable<T> USS<T>(this IEnumerable<T> e, string A, string B, string C, string D)
        where T : VisualElement
    {
        return e.ForEachAndReturn(x => x.USS(A, B, C, D));
    }

    private static IEnumerable<T> ForEachAndReturn<T>(this IEnumerable<T> source, Action<T> action)
    {
